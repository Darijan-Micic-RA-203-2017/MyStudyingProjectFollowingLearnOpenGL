...
void cursor_pos_callback_for_basic_lighting_specular(GLFWwindow* window, double xpos, double ypos)
{
	...
	{
		previousCursorPosX_for_3_2_2 = static_cast<float>(xpos);
		previousCursorPosY_for_3_2_2 = static_cast<float>(ypos);
		firstMouseEntry_for_3_2_2 = false;
	}

	// 1. step: calculate the mouse's offset since last frame.
	float xOffset = static_cast<float>(xpos) - previousCursorPosX_for_3_2_2;
	// Order of subtraction is reversed, because y-coordinates range from bottom to top.
	float yOffset = previousCursorPosY_for_3_2_2 - static_cast<float>(ypos);
	previousCursorPosX_for_3_2_2 = static_cast<float>(xpos);
	previousCursorPosY_for_3_2_2 = static_cast<float>(ypos);
