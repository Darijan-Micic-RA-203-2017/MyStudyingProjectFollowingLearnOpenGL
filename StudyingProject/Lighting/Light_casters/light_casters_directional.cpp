...
float previousCursorPosX_for_3_5_1 = (float) window_width / 2.0f;
float previousCursorPosY_for_3_5_1 = (float) window_height / 2.0f;
...
float deltaTime_for_3_5_1 = 0.0f;
// The time it took to render the previous frame.
float previousFrameTime_for_3_5_1 = 0.0f;
...
int draw_light_casters_directional()
{
	...
	GLFWwindow* window = glfwCreateWindow(window_width, window_height, 
		"Lighting - Light casters, directional", NULL, NULL);
	if (window == NULL)
	{
	...
	glfwSetFramebufferSizeCallback(window, framebuffer_size_callback_for_light_casters_directional);
	glfwSetCursorPosCallback(window, cursor_pos_callback_for_light_casters_directional);
	glfwSetScrollCallback(window, scroll_callback_for_light_casters_directional);
	...
	ShaderProgram ourShaderProgram("Light_casters/vertex_shader_for_3_5_1.glsl", 
		"Light_casters/fragment_shader_for_3_5_1.glsl");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		deltaTime_for_3_5_1 = currentFrameTime - previousFrameTime_for_3_5_1;
		previousFrameTime_for_3_5_1 = currentFrameTime;
		...
		*/
		...
		glm::mat4 projectionMatrix = glm::perspective(glm::radians(camera_for_3_5_1.fov), 
		...
		glm::mat4 viewMatrix = camera_for_3_5_1.getCalculatedViewMatrix();
	...
	firstMouseEntry_for_3_5_1 = true;
...
void cursor_pos_callback_for_light_casters_directional(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (firstMouseEntry_for_3_5_1)
	{
		previousCursorPosX_for_3_5_1 = xPos;
		previousCursorPosY_for_3_5_1 = yPos;
		firstMouseEntry_for_3_5_1 = false;
	}

	// 1. step: calculate the mouse's offset since last frame.
	float xOffset = xPos - previousCursorPosX_for_3_5_1;
	// Order of subtraction is reversed, because y-coordinates range from bottom to top.
	float yOffset = previousCursorPosY_for_3_5_1 - yPos;
	previousCursorPosX_for_3_5_1 = xPos;
	previousCursorPosY_for_3_5_1 = yPos;

	// 2. step onward: done in Camera class.
	camera_for_3_5_1.processMouseMovement(xOffset, yOffset);
}
...
{
	camera_for_3_5_1.processMouseScroll(static_cast<float>(yoffset));
}
...
void processInput_for_light_casters_directional(GLFWwindow* window)
{
	...
	{
		camera_for_3_5_1.processKeyboardInput("W", deltaTime_for_3_5_1);
	}
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
	{
		camera_for_3_5_1.processKeyboardInput("S", deltaTime_for_3_5_1);
	}
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
	{
		camera_for_3_5_1.processKeyboardInput("A", deltaTime_for_3_5_1);
	}
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
	{
		camera_for_3_5_1.processKeyboardInput("D", deltaTime_for_3_5_1);
	}
}
