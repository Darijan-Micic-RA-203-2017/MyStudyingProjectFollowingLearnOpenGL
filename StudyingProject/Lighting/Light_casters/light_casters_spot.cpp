...
float previousCursorPosX_for_3_5_3 = (float) window_width / 2.0f;
float previousCursorPosY_for_3_5_3 = (float) window_height / 2.0f;
...
float deltaTime_for_3_5_3 = 0.0f;
// The time it took to render the previous frame.
float previousFrameTime_for_3_5_3 = 0.0f;

int draw_light_casters_spot()
{
	...
	GLFWwindow* window = glfwCreateWindow(window_width, window_height, 
		"Lighting - Light casters, spot", NULL, NULL);
	if (window == NULL)
	{
	...
	glfwSetFramebufferSizeCallback(window, framebuffer_size_callback_for_light_casters_spot);
	glfwSetCursorPosCallback(window, cursor_pos_callback_for_light_casters_spot);
	glfwSetScrollCallback(window, scroll_callback_for_light_casters_spot);
	...
	ShaderProgram ourShaderProgram("Light_casters/vertex_shader_for_3_5_3.glsl", 
		"Light_casters/fragment_shader_for_3_5_3.glsl");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		deltaTime_for_3_5_3 = currentFrameTime - previousFrameTime_for_3_5_3;
		previousFrameTime_for_3_5_3 = currentFrameTime;

		// First part: Process the user's input.
		processInput_for_light_casters_spot(window);
		...
		*/
		...
		glm::mat4 projectionMatrix = glm::perspective(glm::radians(camera_for_3_5_3.fov), 
		...
		glm::mat4 viewMatrix = camera_for_3_5_3.getCalculatedViewMatrix();
		...
		ourShaderProgram.setFloatVec3Uniform("positionOfViewer", camera_for_3_5_3.cameraPosition);
		...
		*/
		...
		modelMatrix = glm::mat4(1.0f);
		modelMatrix = glm::translate(modelMatrix, positionOfLightSource_for_3_5_3);
	...
	firstMouseEntry_for_3_5_3 = true;
...
void framebuffer_size_callback_for_light_casters_spot(GLFWwindow* window, int width, int height)
{
	glViewport(0, 0, width, height);
}

// Function that will be called every time the user moves the mouse while the application has focus.
void cursor_pos_callback_for_light_casters_spot(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (firstMouseEntry_for_3_5_3)
	{
		previousCursorPosX_for_3_5_3 = xPos;
		previousCursorPosY_for_3_5_3 = yPos;
		firstMouseEntry_for_3_5_3 = false;
	}

	// 1. step: calculate the mouse's offset since last frame.
	float xOffset = xPos - previousCursorPosX_for_3_5_3;
	// Order of subtraction is reversed, because y-coordinates range from bottom to top.
	float yOffset = previousCursorPosY_for_3_5_3 - yPos;
	previousCursorPosX_for_3_5_3 = xPos;
	previousCursorPosY_for_3_5_3 = yPos;

	// 2. step onward: done in Camera class.
	camera_for_3_5_3.processMouseMovement(xOffset, yOffset);
}

// Function that will be called every time the user scrolls the mouse's middle button.
void scroll_callback_for_light_casters_spot(GLFWwindow* window, double xoffset, double yoffset)
{
	camera_for_3_5_3.processMouseScroll(static_cast<float>(yoffset));
}

// Input processing function.
void processInput_for_light_casters_spot(GLFWwindow* window)
{
	...
	{
		camera_for_3_5_3.processKeyboardInput("W", deltaTime_for_3_5_3);
	}
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
	{
		camera_for_3_5_3.processKeyboardInput("S", deltaTime_for_3_5_3);
	}
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
	{
		camera_for_3_5_3.processKeyboardInput("A", deltaTime_for_3_5_3);
	}
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
	{
		camera_for_3_5_3.processKeyboardInput("D", deltaTime_for_3_5_3);
	}
}
