...
glm::vec3 positionOfLightSource_for_3_4_1(1.2f, 1.0f, 2.0f);

// All setting are kept in an instance of the camera class.
Camera camera_for_3_4_1(glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f));

bool firstMouseEntry_for_3_4_1 = true;
float previousCursorPosX_for_3_4_1 = (float) window_width / 2.0f;
float previousCursorPosY_for_3_4_1 = (float) window_height / 2.0f;
...
float deltaTime_for_3_4_1 = 0.0f;
// The time it took to render the previous frame.
float previousFrameTime_for_3_4_1 = 0.0f;
...
int draw_lighting_maps_diffuse()
{
	...
	GLFWwindow* window = glfwCreateWindow(window_width, window_height, 
		"Lighting - Lighting maps, diffuse", NULL, NULL);
	if (window == NULL)
	{
	...
	glfwSetFramebufferSizeCallback(window, framebuffer_size_callback_for_lighting_maps_diffuse);
	glfwSetCursorPosCallback(window, cursor_pos_callback_for_lighting_maps_diffuse);
	glfwSetScrollCallback(window, scroll_callback_for_lighting_maps_diffuse);
	...
	ShaderProgram ourShaderProgram("Lighting_maps/vertex_shader_for_3_4_1.glsl", 
		"Lighting_maps/fragment_shader_for_3_4_1.glsl");
	...
	// Generate texture, set its wrapping and filtering parameters, load image-to-become-texture from file system
	// and generate all the required mipmaps using helper class.
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		deltaTime_for_3_4_1 = currentFrameTime - previousFrameTime_for_3_4_1;
		previousFrameTime_for_3_4_1 = currentFrameTime;

		// First part: Process the user's input.
		processInput_for_lighting_maps_diffuse(window);
		...
		positionOfLightSource_for_3_4_1.x = sin(time) * 2.0f + 1.0f;
		positionOfLightSource_for_3_4_1.y = sin(time / 2.0f);
		*/
		...
		glm::mat4 projectionMatrix = glm::perspective(glm::radians(camera_for_3_4_1.fov), 
		...
		glm::mat4 viewMatrix = camera_for_3_4_1.getCalculatedViewMatrix();
		...
		ourShaderProgram.setFloatVec3Uniform("positionOfViewer", camera_for_3_4_1.cameraPosition);
		...
		*/
		...
		modelMatrix = glm::translate(modelMatrix, positionOfLightSource_for_3_4_1);
...
void cursor_pos_callback_for_lighting_maps_diffuse(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (firstMouseEntry_for_3_4_1)
	{
		previousCursorPosX_for_3_4_1 = static_cast<float>(xpos);
		previousCursorPosY_for_3_4_1 = static_cast<float>(ypos);
		firstMouseEntry_for_3_4_1 = false;
	}

	// 1. step: calculate the mouse's offset since last frame.
	float xOffset = static_cast<float>(xpos) - previousCursorPosX_for_3_4_1;
	// Order of subtraction is reversed, because y-coordinates range from bottom to top.
	float yOffset = previousCursorPosY_for_3_4_1 - static_cast<float>(ypos);
	previousCursorPosX_for_3_4_1 = static_cast<float>(xpos);
	previousCursorPosY_for_3_4_1 = static_cast<float>(ypos);

	// 2. step onward: done in Camera class.
	camera_for_3_4_1.processMouseMovement(xOffset, yOffset);
}
...
{
	camera_for_3_4_1.processMouseScroll(static_cast<float>(yoffset));
}
...
void processInput_for_lighting_maps_diffuse(GLFWwindow* window)
{
	...
	{
		camera_for_3_4_1.processKeyboardInput("W", deltaTime_for_3_4_1);
	}
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
	{
		camera_for_3_4_1.processKeyboardInput("S", deltaTime_for_3_4_1);
	}
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
	{
		camera_for_3_4_1.processKeyboardInput("A", deltaTime_for_3_4_1);
	}
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
	{
		camera_for_3_4_1.processKeyboardInput("D", deltaTime_for_3_4_1);
	}
}
