#include "lighting_maps_exercise2.h"

const int window_width = 800;
const int window_height = 600;

// Position of light source in world-space coordinates.
glm::vec3 positionOfLightSource_for_3_4_3(1.2f, 1.0f, 2.0f);

// All setting are kept in an instance of the camera class.
Camera camera_for_3_4_3(glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f));

bool firstMouseEntry_for_3_4_3 = true;
float previousCursorPosX_for_3_4_3 = (float) window_width / 2.0f;
float previousCursorPosY_for_3_4_3 = (float) window_height / 2.0f;
...
float deltaTime_for_3_4_3 = 0.0f;
// The time it took to render the previous frame.
float previousFrameTime_for_3_4_3 = 0.0f;

int draw_lighting_maps_exercise2()
{
	...
	GLFWwindow* window = glfwCreateWindow(window_width, window_height, 
		"Lighting - Lighting maps, exercise 2", NULL, NULL);
	if (window == NULL)
	{
	...
	glfwSetFramebufferSizeCallback(window, framebuffer_size_callback_for_lighting_maps_exercise2);
	glfwSetCursorPosCallback(window, cursor_pos_callback_for_lighting_maps_exercise2);
	glfwSetScrollCallback(window, scroll_callback_for_lighting_maps_exercise2);
	...
	ShaderProgram ourShaderProgram("Lighting_maps/vertex_shader_for_3_4_3.glsl", 
		"Lighting_maps/fragment_shader_for_3_4_3.glsl");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		deltaTime_for_3_4_3 = currentFrameTime - previousFrameTime_for_3_4_3;
		previousFrameTime_for_3_4_3 = currentFrameTime;

		// First part: Process the user's input.
		processInput_for_lighting_maps_exercise2(window);
		...
		positionOfLightSource_for_3_4_3.x = sin(time) * 2.0f + 1.0f;
		positionOfLightSource_for_3_4_3.y = sin(time / 2.0f);
		*/
		...
		glm::mat4 projectionMatrix = glm::perspective(glm::radians(camera_for_3_4_3.fov), 
		...
		glm::mat4 viewMatrix = camera_for_3_4_3.getCalculatedViewMatrix();
		...
		ourShaderProgram.setFloatVec3Uniform("positionOfViewer", camera_for_3_4_3.cameraPosition);

		// Set position of light source to global variable "positionOfLightSource".
		ourShaderProgram.setFloatVec3Uniform("lightSource.position", positionOfLightSource_for_3_4_3);
		...
		*/
		...
		modelMatrix = glm::translate(modelMatrix, positionOfLightSource_for_3_4_3);
...
void framebuffer_size_callback_for_lighting_maps_exercise2(GLFWwindow* window, int width, int height)
{
	glViewport(0, 0, width, height);
}

// Function that will be called every time the user moves the mouse while the application has focus.
void cursor_pos_callback_for_lighting_maps_exercise2(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (firstMouseEntry_for_3_4_3)
	{
		previousCursorPosX_for_3_4_3 = static_cast<float>(xpos);
		previousCursorPosY_for_3_4_3 = static_cast<float>(ypos);
		firstMouseEntry_for_3_4_3 = false;
	}

	// 1. step: calculate the mouse's offset since last frame.
	float xOffset = static_cast<float>(xpos) - previousCursorPosX_for_3_4_3;
	// Order of subtraction is reversed, because y-coordinates range from bottom to top.
	float yOffset = previousCursorPosY_for_3_4_3 - static_cast<float>(ypos);
	previousCursorPosX_for_3_4_3 = static_cast<float>(xpos);
	previousCursorPosY_for_3_4_3 = static_cast<float>(ypos);

	// 2. step onward: done in Camera class.
	camera_for_3_4_3.processMouseMovement(xOffset, yOffset);
}

// Function that will be called every time the user scrolls the mouse's middle button.
void scroll_callback_for_lighting_maps_exercise2(GLFWwindow* window, double xoffset, double yoffset)
{
	camera_for_3_4_3.processMouseScroll(static_cast<float>(yoffset));
}

// Input processing function.
void processInput_for_lighting_maps_exercise2(GLFWwindow* window)
{
	...
	{
		camera_for_3_4_3.processKeyboardInput("W", deltaTime_for_3_4_3);
	}
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
	{
		camera_for_3_4_3.processKeyboardInput("S", deltaTime_for_3_4_3);
	}
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
	{
		camera_for_3_4_3.processKeyboardInput("A", deltaTime_for_3_4_3);
	}
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
	{
		camera_for_3_4_3.processKeyboardInput("D", deltaTime_for_3_4_3);
	}
}
