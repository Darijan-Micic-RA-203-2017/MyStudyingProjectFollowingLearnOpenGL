...
glm::vec3 positionOfLightSource_for_3_3_1(1.2f, 1.0f, 2.0f);

// All setting are kept in an instance of the camera class.
Camera camera_for_3_3_1(glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f));

bool firstMouseEntry_for_3_3_1 = true;
float previousCursorPosX_for_3_3_1 = (float) window_width / 2.0f;
float previousCursorPosY_for_3_3_1 = (float) window_height / 2.0f;
...
float deltaTime_for_3_3_1 = 0.0f;
// The time it took to render the previous frame.
float previousFrameTime_for_3_3_1 = 0.0f;
...
int draw_materials()
{
	...
	glfwSetFramebufferSizeCallback(window, framebuffer_size_callback_for_materials);
	glfwSetCursorPosCallback(window, cursor_pos_callback_for_materials);
	glfwSetScrollCallback(window, scroll_callback_for_materials);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		deltaTime_for_3_3_1 = currentFrameTime - previousFrameTime_for_3_3_1;
		previousFrameTime_for_3_3_1 = currentFrameTime;

		// First part: Process the user's input.
		processInput_for_materials(window);
		...
		positionOfLightSource_for_3_3_1.x = sin(time) * 2.0f + 1.0f;
		positionOfLightSource_for_3_3_1.y = sin(time / 2.0f);
		*/
		...
		glm::mat4 projectionMatrix = glm::perspective(glm::radians(camera_for_3_3_1.fov), 
		...
		glm::mat4 viewMatrix = camera_for_3_3_1.getCalculatedViewMatrix();
		...
		modelMatrix = glm::translate(modelMatrix, positionOfLightSource_for_3_3_1);
...
void framebuffer_size_callback_for_materials(GLFWwindow* window, int width, int height)
{
	glViewport(0, 0, width, height);
}

// Function that will be called every time the user moves the mouse while the application has focus.
void cursor_pos_callback_for_materials(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (firstMouseEntry_for_3_3_1)
	{
		previousCursorPosX_for_3_3_1 = static_cast<float>(xpos);
		previousCursorPosY_for_3_3_1 = static_cast<float>(ypos);
		firstMouseEntry_for_3_3_1 = false;
	}

	// 1. step: calculate the mouse's offset since last frame.
	float xOffset = static_cast<float>(xpos) - previousCursorPosX_for_3_3_1;
	// Order of subtraction is reversed, because y-coordinates range from bottom to top.
	float yOffset = previousCursorPosY_for_3_3_1 - static_cast<float>(ypos);
	previousCursorPosX_for_3_3_1 = static_cast<float>(xpos);
	previousCursorPosY_for_3_3_1 = static_cast<float>(ypos);

	// 2. step onward: done in Camera class.
	camera_for_3_3_1.processMouseMovement(xOffset, yOffset);
}

// Function that will be called every time the user scrolls the mouse's middle button.
void scroll_callback_for_materials(GLFWwindow* window, double xoffset, double yoffset)
{
	camera_for_3_3_1.processMouseScroll(static_cast<float>(yoffset));
}

// Input processing function.
void processInput_for_materials(GLFWwindow* window)
{
	...
	{
		camera_for_3_3_1.processKeyboardInput("W", deltaTime_for_3_3_1);
	}
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
	{
		camera_for_3_3_1.processKeyboardInput("S", deltaTime_for_3_3_1);
	}
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
	{
		camera_for_3_3_1.processKeyboardInput("A", deltaTime_for_3_3_1);
	}
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
	{
		camera_for_3_3_1.processKeyboardInput("D", deltaTime_for_3_3_1);
	}
}
