#include "materials_exercise1.h"

const int window_width = 800;
const int window_height = 600;

// Position of light source in world-space coordinates.
glm::vec3 positionOfLightSource_for_3_3_2(1.2f, 1.0f, 2.0f);

// All settings are kept in an instance of the camera class.
Camera camera_for_3_3_2(glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f));

bool firstMouseEntry_for_3_3_2 = true;
float previousCursorPosX_for_3_3_2 = (float) window_width / 2.0f;
float previousCursorPosY_for_3_3_2 = (float) window_height / 2.0f;
...
float deltaTime_for_3_3_2 = 0.0f;
// The time it took to render the previous frame.
float previousFrameTime_for_3_3_2 = 0.0f;
...
int draw_materials_exercise1()
{
	...
	GLFWwindow* window = glfwCreateWindow(window_width, window_height, 
		"Lighting - Materials, exercise 1", NULL, NULL);
	if (window == NULL)
	{
	...
	glfwSetFramebufferSizeCallback(window, framebuffer_size_callback_for_materials_exercise1);
	glfwSetCursorPosCallback(window, cursor_pos_callback_for_materials_exercise1);
	glfwSetScrollCallback(window, scroll_callback_for_materials_exercise1);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		deltaTime_for_3_3_2 = currentFrameTime - previousFrameTime_for_3_3_2;
		previousFrameTime_for_3_3_2 = currentFrameTime;

		// First part: Process the user's input.
		processInput_for_materials_exercise1(window);
		...
		positionOfLightSource_for_3_3_2.x = glm::sin(time) * 2.0f + 1.0f;
		positionOfLightSource_for_3_3_2.y = glm::sin(time / 2.0f);
		*/
		...
		*/
		...
		ourShaderProgram.setFloatVec3Uniform("positionOfViewer", camera_for_3_3_2.cameraPosition);
	...
	firstMouseEntry_for_3_3_2 = true;
...
void cursor_pos_callback_for_materials_exercise1(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (firstMouseEntry_for_3_3_2)
	{
		previousCursorPosX_for_3_3_2 = xPos;
		previousCursorPosY_for_3_3_2 = yPos;
		firstMouseEntry_for_3_3_2 = false;
	}

	// 1. step: calculate the mouse's offset since last frame.
	float xOffset = xPos - previousCursorPosX_for_3_3_2;
	// Order of subtraction is reversed, because y-coordinates range from bottom to top.
	float yOffset = previousCursorPosY_for_3_3_2 - yPos;
	previousCursorPosX_for_3_3_2 = xPos;
	previousCursorPosY_for_3_3_2 = yPos;

	// 2. step onward: done in Camera class.
	camera_for_3_3_2.processMouseMovement(xOffset, yOffset);
}
...
{
	camera_for_3_3_2.processMouseScroll(static_cast<float>(yoffset));
}
...
void processInput_for_materials_exercise1(GLFWwindow* window)
{
	...
	{
		camera_for_3_3_2.processKeyboardInput("W", deltaTime_for_3_3_2);
	}
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
	{
		camera_for_3_3_2.processKeyboardInput("S", deltaTime_for_3_3_2);
	}
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
	{
		camera_for_3_3_2.processKeyboardInput("A", deltaTime_for_3_3_2);
	}
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
	{
		camera_for_3_3_2.processKeyboardInput("D", deltaTime_for_3_3_2);
	}
}
