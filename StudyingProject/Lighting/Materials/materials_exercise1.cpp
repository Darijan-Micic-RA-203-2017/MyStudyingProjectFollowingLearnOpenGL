...
int draw_materials_exercise1()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		positionOfLightSource_for_3_3_2.x = sin(time) * 2.0f + 1.0f;
...
void cursor_pos_callback_for_materials_exercise1(GLFWwindow* window, double xpos, double ypos)
{
	...
	{
		previousCursorPosX_for_3_3_2 = static_cast<float>(xpos);
		previousCursorPosY_for_3_3_2 = static_cast<float>(ypos);
		firstMouseEntry_for_3_3_2 = false;
	}

	// 1. step: calculate the mouse's offset since last frame.
	float xOffset = static_cast<float>(xpos) - previousCursorPosX_for_3_3_2;
	// Order of subtraction is reversed, because y-coordinates range from bottom to top.
	float yOffset = previousCursorPosY_for_3_3_2 - static_cast<float>(ypos);
	previousCursorPosX_for_3_3_2 = static_cast<float>(xpos);
	previousCursorPosY_for_3_3_2 = static_cast<float>(ypos);
